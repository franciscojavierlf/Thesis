...
ORDERER_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/ciencia.edu/orderers/orderer.ciencia.edu/msp/tlscacerts/tlsca.ciencia.edu-cert.pem
...
setOrdererGlobals() {
  ...
  CORE_PEER_TLS_ROOTCERT_FILE=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/ciencia.edu/orderers/orderer.ciencia.edu/msp/tlscacerts/tlsca.ciencia.edu-cert.pem
  CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/ordererOrganizations/ciencia.edu/users/Admin@ciencia.edu/msp
}
...
updateAnchorPeers() {
  ...
  if [ $CHANNEL -eq 1 ]; then
    set -x
    peer channel update -o orderer.ciencia.edu:7050 -c channel1 -f ./channel-artifacts/${CORE_PEER_LOCALMSPID}anchors.tx --tls --cafile $ORDERER_CA >&log.txt
    res=$?
    set +x
  else
    set -x
    peer channel update -o orderer.ciencia.edu:7050 -c channel2 -f ./channel-artifacts/${CORE_PEER_LOCALMSPID}anchors_channel2.tx --tls --cafile $ORDERER_CA >&log.txt
    res=$?
    set +x    
  fi
  cat log.txt
...
commitChaincodeDefinition() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer lifecycle chaincode commit -o orderer.ciencia.edu:7050 --channelID $CHANNEL_NAME --name mycc $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} --init-required >&log.txt
    res=$?
    set +x
  else
    set -x
    peer lifecycle chaincode commit -o orderer.ciencia.edu:7050 --tls --cafile $ORDERER_CA --channelID $CHANNEL_NAME --name mycc $PEER_CONN_PARMS --version ${VERSION} --sequence ${VERSION} --init-required >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
...
fetchChannelConfig() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    set -x
    peer channel fetch config config_block.pb -o orderer.ciencia.edu:7050 -c $CHANNEL --cafile $ORDERER_CA
    set +x
  else
    set -x
    peer channel fetch config config_block.pb -o orderer.ciencia.edu:7050 -c $CHANNEL --tls --cafile $ORDERER_CA
    set +x
  fi
...
chaincodeInvoke() {
  ...
  if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
    echo "+++ invoke 1 ++++ "
    set -x
    peer chaincode invoke -o orderer.ciencia.edu:7050 -C $CHANNEL_NAME -n mycc $PEER_CONN_PARMS ${INIT_ARG} -c ${CCARGS} >&log.txt
    res=$?
    set +x
  else
    echo "+++ invoke 2 ++++ "
    set -x
    peer chaincode invoke -o orderer.ciencia.edu:7050 --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc $PEER_CONN_PARMS ${INIT_ARG} -c ${CCARGS} >&log.txt
    res=$?
    set +x
  fi
  cat log.txt
